...
export const facultySignIn = (formData, navigate) => async (dispatch) => {
  try {
    ...
    if (data.result.email) navigate("/faculty/home");
...
export const facultySignUp = (formData, navigate) => async (dispatch) => {
  try {
    const { data } = await api.facultySignUp(formData);
    dispatch({ type: FACULTY_REGISTER, data });
    if (data.result) navigate("/login/facultylogin");
    else navigate("/faculty/password");
  } catch (error) {
    dispatch({ type: SET_ERRORS, payload: error.response.data });
  }
